...
# Refined LIGHT Input MicrobeAnnotator
...
for (sheet_name in names(df_list)) {
  ...
  FULL_DB_Anno[[sheet_name]] <- c(matched_kofam, matched_swiss, no_matches)
  FULL_DB_Anno <- as.data.frame(FULL_DB_Anno)
}

FULL_DB_Anno <- t(FULL_DB_Anno)
colnames(FULL_DB_Anno) <- c("Kofam", "Swissprot", "NA")
FULL_DB_Anno <- as.data.frame(FULL_DB_Anno)
...
for (sheet_name in names(Taxon_DB)) {
  ...
  png(filename = paste0("Refined_Light_Top_5_Taxa_", sheet_name, ".png"), width = 1400, height = 1000, res = 200, bg = "gray81")
  pie(
    top_taxa,
    labels = lbls,
  ...
  title(main = paste("Refined Light Top 5 Taxa (Overall) - ", sheet_name),
        col.main = "black", cex.main = 1.5, line = -1)
  dev.off()
}
...
for (sheet_name in names(df_list)) {
  ...
  clean_no_hit_ids <- clean_df$query_id[clean_df$database == "NA"]
  
  clean_venn_input <- list(
    KOfam = clean_kofam_ids,
    SwissProt = clean_swissprot_ids,
    NoMatch = clean_no_hit_ids
  )
  
  venn_plot <- ggVennDiagram(clean_venn_input, label = "both") +
    ggtitle(paste("Refined Light Annotation Databases Distribution: ", sheet_name)) +
    theme(plot.title = element_text(hjust = 0.5, face = "bold"))
  
  ggsave(
    filename = paste0("Refined_Light_Annotation_Venn_", sheet_name, ".png"),
    plot = venn_plot,
    width = 8,
    height = 8,
    bg = "gray87", 
...
# KNOWN: Top 5 Products Swissprot, Column C.
...
for (sheet_name in names(df_list)){
  ...
  ColC_bar_data <- ggplot(Known_Products_ColC[[sheet_name]], aes(x = reorder(Known_Product, Freq.), y = Freq.)) +
    ...
    geom_text(aes(label = Freq.), 
              ...
              vjust = 0.5, hjust = 0, angle = 0, color = "black", size = 3.5) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 1, face = "bold"),
          plot.margin = margin(t = 5, r = 5, b = 5, l = 5)
    ) + 
    labs(title = paste0("Col. C: Refined Light Top 5 Protein Products Counts - ", sheet_name))
  
  ggsave(paste0("Col. C: Refined_light_Top_5_Protein_Products", sheet_name, ".png"), plot = ColC_bar_data,
         width = 8, height = 6, units = "in", bg = "gray87", dpi = 300)
}
...
for(sheet_name in names(df_list)){
  ...
  Hypo_bar_data <- ggplot(Final_Hypo[[sheet_name]], aes(x = reorder(Hypo_protein, Freq.), y = Freq.)) +
    ...
    labs(title = paste0("Col. C: Refined Light Top 5 Hypothetical/Uncharacteristic Proteins - ", sheet_name))
  
  ggsave(paste0("Col. C: Refined_light_Protein_Unchar_Product_", sheet_name, ".png"), plot = Hypo_bar_data,
         width = 8, height = 6, units = "in", bg = "gray87", dpi = 300)
}
...
# KNOWN: Pulling top 5 from Ko Product Kofam Database, Column E
Final_KO <- list()
...
for (sheet_name in names(df_list)){
  ...
  KO_bar_data <- ggplot(Final_KO[[sheet_name]], aes(x = reorder(KO_Product, Freq.), y = Freq.)) +
    ...
    labs(title = paste0("Col. E: Refined Light Top 5 Known Products - ", sheet_name))
  
  ggsave(paste0("Col. E: Refined Light Top 5 Known Products ", sheet_name, ".png"), plot = KO_bar_data,
         width = 8, height = 6, units = "in", bg = "gray87", dpi = 300)
}
...
for(sheet_name in names(df_list)){
  ...
  Hypo_bar_data_ColE <- ggplot(Final_Col_E[[sheet_name]], aes(x = reorder(Hypo_protein_ColE, Freq.), y = Freq.)) +
    ...
    labs(title = paste0("Col. E: Refined Light Hypothetical/Uncharacteristic KO Proteins - ", sheet_name))
  
  ggsave(paste0("Col. E: Refined Light Top 5 Unknown Products ", sheet_name, ".png"), plot = Hypo_bar_data_ColE,
         width = 8, height = 6, units = "in", bg = "gray87", dpi = 300)
}

# Summary 
MicrobeAnnotator_refined_light_summary <- data.table(
  ...
  Annotation_counts = FULL_DB_Anno
)


# Write Excel
excel_summary <- list("LIGHT REFINED Summary" = MicrobeAnnotator_refined_light_summary)
...
write_xlsx(excel_summary, "Refined_Light_Summary.xlsx")
